...
**Consequences**
DDL-MKT-001 through DDL-MKT-008 describe a page that no longer exists and require review under §7.5 once this record is resolved. The frozen Homepage v1.0.0 release is superseded and its governance status needs restating. `docs/archive/homepage-program/HOMEPAGE_STORYBOARD.md`, `docs/archive/homepage-program/HOMEPAGE_RELEASE.md`, and the untracked `tests/homepage-scene-0*.test.tsx` files refer to the removed implementation. The marketing surface now owns its own radius, type, and color scales, which must not be folded back into the product tokens.
