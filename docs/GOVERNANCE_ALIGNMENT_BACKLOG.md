# Hirevo — Governance Alignment Backlog
...
1. ~~**`GAB-D1` runs first.**~~ **Done 5 Aug 2026.** It was the only item in this backlog that failed a blocking gate (Checklist §8.9, Gate 2), and until it was fixed any review touching report generation terminated unscored. That constraint is lifted; nothing in this backlog blocks review work now.
...
**Problem Summary**
Two problems, and the second is the more serious. The line reads `> The single source of truth for Hirevo — an AI-powered hiring intelligence platform.` The positioning is the pre-governance framing `DDL-POS-001` replaced. But it also claims to be the single source of truth, which is now false — governance is — and it does not link to either governing document. A contributor entering here is directed away from the documents that bind them.
...
This is the **frozen Hirevo v1.0 legacy app**, which is why the obvious fix is not the recommended one.
...
| **Status** | **Done** — 5 Aug 2026, verified against generated PDFs |
...
**Required Action**
Set `author="Hirevo"`. Change the footer to `"Generated by Hirevo • Confidential"`. **Remove the vendor attribution entirely** — do not relocate it to a metadata field, a settings page, or an about screen. The prohibition is on naming the supplier as a quality signal anywhere a user sees it, not on its position.

**Run this item first.** It is the only gate failure in the backlog.

**Resolution — 5 Aug 2026.** Done exactly as required, and no further.
`author="Hirevo"` on both documents (was lines 218/409); both footers now read
`"Generated by Hirevo  •  Confidential"` (was 359/563, since shifted by an
inline comment recording *why* the vendor name is absent, so a future edit does
not restore it as a courtesy). **The vendor attribution was removed, not
relocated** — it is not in the metadata, not in a settings screen, not on an
about page, and not anywhere else a customer can see.

Two further reasons the prohibition is right here, recorded because they are
engineering reasons rather than governance ones and would otherwise be lost:
the platform routes across providers by configuration
(`app/ai/gateway/`), so **a vendor name printed on an artifact silently becomes
false the moment the gateway fails over** — the PDF outlives the request that
made it and cannot be corrected once sent; and an exported PDF is the AI output
most likely to leave the building, so it is the worst place to carry a claim
nobody re-checks.

**The rule is now self-enforcing.** `GAB-D1` was found by reading, not by a
failing test, and the whole backend suite stayed green across the fix because
**nothing had ever asserted on that footer.** Two guards close that:
...
docs, and the skills taxonomy — candidates list it on their résumés
(`app/nlp/skill_normalizer.py`). A repo-wide ban would be wrong and would be
deleted the first time it fired. Neither is a snapshot: each asserts what must be
**absent** plus the one attribution that must be **present**, so wording and
layout stay free to change, and deleting the attribution fails too.

Two properties worth keeping. `TestEveryExporterIsCovered` fails if
`report_generator` grows a public `generate_*` the guard does not exercise — a
new exporter would otherwise inherit the footer by copy-paste and be tested by
nothing, which is how this happened. And each guard **was watched failing**: the
exact removed string was reintroduced in both codebases, the right assertions
fired, and both files were restored byte-for-byte. A guard never seen to fail is
not a guard.

**Verified, not merely implemented.** Both PDFs were generated and inspected —
rendered text *and* document metadata (the Properties pane, which is what
`author=` reaches): no `Groq`, `Powered by`, `AI Resume Intelligence Platform`,
or any other vendor string; `author=Hirevo`; footer present on both; the
`•` glyph intact (U+2022). Backend suite **493 passed**, unchanged.

**Deliberately NOT done in this item.** The PDF `title=` strings — `"ATS
Analysis Report"` and **`"AI Recruiter Match Report"`** (lines 217/408) — still
carry the category-naming drift this item's Problem Summary mentions. They are
outside the Required Action, which names `author` and the footer only, and
`"AI Recruiter Match Report"` is customer-visible in a PDF reader's title bar.
**This needs its own item or an explicit extension of this one** — it was not
changed unilaterally, because renaming a document a customer may already have
filed is a decision, not a cleanup. Related: `GAB-D2` covers the same drift in
...
**Required Action**
`title="Hirevo API"` · `description="Backend API for Hirevo — Decision Intelligence for hiring."` Confirm nothing downstream pins the current title string before changing it.
...
| GAB-D1 | D | Major **⚠ GATE** | `report_generator.py` | S | AI Owner | **Done** (5 Aug 2026) |
...
**Totals:** 11 actionable (**1 done**) · 2 blocked on decisions · **0 open gate
failures** — `GAB-D1` was the only one and closed on 5 Aug 2026, so **review work
touching report generation is no longer terminated unscored** · 1 closed.
...
*Hirevo Governance Alignment Backlog v1.0 — execution document, disposable, non-authoritative.*
