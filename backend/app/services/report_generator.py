...
# ── Shared page decorations (watermark + per-page footer) ────────────────────
#
# Drawn via SimpleDocTemplate's onFirstPage/onLaterPages hooks, which paint
# BEFORE the flowables — so the watermark sits underneath the content, never
# over it. One implementation for every exporter; a new report wired without
# these hooks fails tests/test_export_attribution.py.

WATERMARK_TEXT = "HIREVO"
PAGE_FOOTER_TEXT = "Generated by Hirevo · hirevo.in · Page {page}"


def _paint_page_brand(canvas, doc) -> None:
    """Diagonal brand watermark behind the content + footer in the bottom margin."""
    ...
    canvas.drawCentredString(
        page_w / 2, 11 * mm, PAGE_FOOTER_TEXT.format(page=canvas.getPageNumber())
    )
    canvas.restoreState()
...
def generate_report(analysis_data: dict, resume_data: dict) -> bytes:
    ...
    # Build PDF
    doc.build(story, onFirstPage=_paint_page_brand, onLaterPages=_paint_page_brand)
