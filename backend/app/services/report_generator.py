...
def generate_report(analysis_data: dict, resume_data: dict) -> bytes:
    ...
    story.append(Paragraph(
        # No model vendor is named here, or anywhere else a customer can see.
        # Naming the supplier on an artifact that leaves the building presents a
        # procurement choice as a quality signal (Checklist §8.9, Gate 2) — and it
        # is a claim that silently becomes false the moment the gateway routes to
        # a different provider. Provenance, if it is ever needed, is an audit
        # record and must be designed as one; it is not this line. See GAB-D1.
        "Generated by HireLens  •  Confidential",
        styles["footer"],
    ))
