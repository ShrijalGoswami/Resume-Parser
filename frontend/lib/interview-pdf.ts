...
export function exportInterviewPdf(pack: InterviewPack) {
  ...
  <style>
    * { box-sizing: border-box; margin: 0; }
    ...
    /* ── Per-page header safe area (the .sheet wrapper) ──────────────── */
    /* Chrome paints the @top-left/@top-right running header even when the
       print dialog collapses the page margins to zero — so on those exports
       the header landed ON the first content line of pages 2+. Body padding
       cannot fix that (top padding exists only on the FIRST page fragment).
       The one per-page reservation Chromium honours regardless of dialog
       settings is a repeating table header: this wrapper's empty spacer
       thead repeats at the top of EVERY printed page and reserves real
       layout height there, so content always starts below the header zone. */
    table.sheet { width: 100%; border-collapse: collapse; margin: 0; font-size: inherit; }
    table.sheet > thead > tr > td, table.sheet > tbody > tr > td { padding: 0; border: 0; }
    .page-pad td { height: 0; }
    ...
    @page {
      /* Top margin is 8mm (not 12mm): the repeating .page-pad spacer adds
         8mm INSIDE every page, so honoured margins still total 16mm of top
         space — identical to the approved geometry, without ballooning. */
      size: A4;
      margin: 8mm 6mm 16mm;
    ...
    @media print {
      /* Top inset comes from the repeating .page-pad spacer (every page),
         not from body padding (first page only). Sides/bottom stay here. */
      body { margin: 0; max-width: none; padding: 0 8mm 4mm; }
      .page-pad td { height: 8mm; }
      .brandbar, .titleblock, .sec-head, h1 { break-after: avoid; }
      .card, tr, .panel, .glance, .row { break-inside: avoid; }
      /* The .sheet wrapper's single body row spans the whole document and
         MUST stay breakable, or the avoid rule above would forbid every
         page break in the file. */
      table.sheet > tbody > tr, table.sheet > tbody > tr > td { break-inside: auto; }
      p, li { orphans: 3; widows: 3; }
      section { margin-top: 14px; }
    }
  </style></head><body>
    <table class="sheet"><thead class="page-pad"><tr><td></td></tr></thead><tbody><tr><td>
    <header>
      <div class="brandbar">
